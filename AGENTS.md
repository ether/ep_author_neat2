...
`ep_author_neat2` runs inside Etherpad's test harness. From an etherpad checkout that has installed this plugin via `pnpm run plugins i --path ../ep_author_neat2`:
...
* Client: `postAceInit`, `aceSetAuthorStyle`, `aceEditEvent`, `acePostWriteDomLineHTML`
