# Contributing to ep_author_neat2

Thanks for helping improve `ep_author_neat2` — a plugin for [Etherpad](https://etherpad.org/). LLM/Agent contributions are explicitly welcome, provided they follow the rules below.
...
`ep_author_neat2` is a plugin, so you develop it inside a checkout of [ether/etherpad](https://github.com/ether/etherpad).
...
# Install ep_author_neat2 from your local clone
pnpm run plugins i --path ../ep_author_neat2

# Start the dev server (port 9001)
pnpm --filter ep_etherpad-lite run dev
```

Edits in `../ep_author_neat2/` are picked up after a server restart.
...
* Etherpad version, ep_author_neat2 version, browser + OS
* Steps to reproduce
